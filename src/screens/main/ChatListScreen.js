...
export default function ChatListScreen({ navigation }) {const { t: tr } = useLanguage();
    ...
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity
              onPress={() => navigation.navigate('Chat', { chatId: item.id, title: other ? `${other.name} ${other.surname || ''}`.trim() : tr("dash_chats") })}
