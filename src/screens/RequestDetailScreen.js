...
export default function RequestDetailScreen({ route, navigation }) {
  ...
  async function handleDelete() {
    Alert.alert('მოთხოვნის წაშლა', 'ნამდვილად გინდა ამ მოთხოვნის წაშლა? ეს ქმედება შეუქცევადია.', [
      ...
      { text: '🗑 წაშლა', style: 'destructive', onPress: async () => {
        ...
        try {
          ...
          Alert.alert('შეცდომა', e.error || 'წაშლა ვერ მოხდა');
  ...
  async function rejectOffer(offerId) {
    Alert.alert('შეთავაზების უარყოფა', 'ნამდვილად გინდა ამ შეთავაზების უარყოფა?', [
      { text: 'გაუქმება', style: 'cancel' },
      { text: '✕ უარყოფა', style: 'destructive', onPress: async () => {
        ...
        try {
          ...
          Alert.alert('შეცდომა', e.error || 'უარყოფა ვერ მოხდა');
  ...
  const statusLabel = req.status === 'open' ? 'ღია' : (req.status === 'in_progress' || req.status === 'pending') ? 'მიმდინარე' : 'დასრულებული';
  ...
  return (
    ...
    <ScrollView style={{ flex: 1, backgroundColor: C.bg }} contentContainerStyle={{ padding: 16, paddingBottom: 30 }}>
      ...
      {req.media?.length > 0 && (
        <Card>
          <Text style={{ color: C.text, fontWeight: '800', fontSize: 15, marginBottom: 12 }}>📷 ფოტო / ვიდეო</Text>
              ...
              {req.media.map((m, i) => {
                ...
                return (
                  <TouchableOpacity key={i} activeOpacity={0.85} onPress={() => {
                    if (isVideo) { Linking.openURL(m.url).catch(() => {}); }
                    else { setPhotoIdx(imgIdx); setShowPhotos(true); }
                  }}>
                ...
                {offer.status === 'accepted' && (
                  <View>
                    ...
                    {offer.chat?.id && (
                      <Btn
                        title="💬 ჩათი"
                        onPress={() => navigation.navigate('Chat', { chatId: offer.chat.id, title: 'ჩათი' })}
                        small
                        outline
                      />
                    )}
