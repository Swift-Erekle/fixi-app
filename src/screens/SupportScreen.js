import { useLanguage } from "../context/LanguageContext"; // src/screens/SupportScreen.js
...
export default function SupportScreen() {const { t: tr } = useLanguage();
  const { user } = useAuth();
  ...
  async function send() {
    ...
    } catch (e) {Alert.alert('⚠️', e?.error || tr("screens_supportscreen_text_1mxfkl"));} finally
    {setSending(false);}
  }
  ...
  if (!user) return (
    <View style={{ flex: 1, backgroundColor: C.bg, justifyContent: 'center', alignItems: 'center', padding: 24 }}>
      ...
      <Text style={{ color: C.text, fontWeight: '800', fontSize: 16 }}>{tr("screens_supportscreen_text_1oatxx")}</Text>
    </View>);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      <View style={{ backgroundColor: C.surface, borderBottomWidth: 1, borderBottomColor: C.border, padding: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          ...
          <View>
            <Text style={{ color: C.text, fontWeight: '900', fontSize: 16 }}>{tr("support_title")}</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 2 }}>
              ...
              <Text style={{ color: opStatus === 'active' ? C.accent : C.ok, fontSize: 11, fontWeight: '600' }}>
                {opStatus === 'active' ? tr("screens_supportscreen_text_1nl3b3") : tr("screens_supportscreen_text_1qt3sk")}
              </Text>
            </View>
          </View>
        </View>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? 64 : 0}>
        <FlatList
          ...
          <View style={{ padding: 40, alignItems: 'center' }}>
              <Text style={{ fontSize: 52, marginBottom: 14 }}>💬</Text>
              <Text style={{ color: C.text, fontWeight: '800', fontSize: 16, marginBottom: 6 }}>{tr("screens_supportscreen_text_1hvwlk")}</Text>
              <Text style={{ color: C.text2, fontSize: 13, textAlign: 'center' }}>{tr("screens_supportscreen_text_c63aff")}</Text>
            </View>
          }
          renderItem={({ item }) => {
            ...
            return (
              <View style={{ alignSelf: isMe ? 'flex-end' : 'flex-start', maxWidth: '80%', marginBottom: 10 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    ...
                    <Text style={{ color: C.text2, fontSize: 11, fontWeight: '600' }}>{tr("support_title")}</Text>
                  </View>
                }
                <View style={{
        ...
        <View style={{ flexDirection: 'row', padding: 10, paddingHorizontal: 14, borderTopWidth: 1, borderTopColor: C.border, backgroundColor: C.surface, alignItems: 'flex-end', gap: 10 }}>
          <TextInput
            ...
            placeholder={tr("chat_msg_placeholder")} placeholderTextColor={C.text2} multiline
