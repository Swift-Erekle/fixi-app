import { useLanguage } from "../../context/LanguageContext"; // src/screens/auth/ForgotScreen.js
...
export default function ForgotScreen({ navigation }) {const { t: tr } = useLanguage();
  ...
  async function sendCode() {
    if (!email.trim()) return Alert.alert(tr("error"), tr("screens_auth_forgotscreen_text_324g3g"));
    setLoading(true);
    try {
  ...
  async function resetPass() {
    ...
    } catch (e) {Alert.alert(tr("error"), e.error || tr("screens_auth_forgotscreen_text_fway7m"));} finally
    {setLoading(false);}
  }
      ...
      <View style={{ flex: 1, justifyContent: 'center', padding: 24 }}>
        ...
        <View style={{ alignItems: 'center', marginBottom: 36 }}>
          ...
          <Text style={{ color: C.text2, fontSize: 13, marginTop: 6, textAlign: 'center' }}>
            {step === 1 ? tr("screens_auth_forgotscreen_text_1emevy") : tr('forgot_code_sent_to', { email })}
          </Text>
        </View>
        ...
        <Card>
          ...
          <>
              ...
              <View style={{ position: 'relative', marginBottom: 16 }}>
                ...
                placeholder={tr("reg_pass_ph")} placeholderTextColor={C.text2}
