// src/screens/AdminSupportChatScreen.js
...
export default function AdminSupportChatScreen({ route, navigation }) {
  ...
  useFocusEffect(useCallback(() => {
    navigation.setOptions({ title: userName ? `🎧 ${userName}` : '🎧 სუპორტი' });
    load();
  ...
  async function send() {
    ...
    try {
      ...
      if (sock?.connected) {
        ...
        Alert.alert('⚠️', 'კავშირი დაიკარგა — სცადე ცოტა მოგვიანებით');
      }
      ...
      <KeyboardAvoidingView style={{ flex:1 }} behavior={Platform.OS==='ios'?'padding':undefined} keyboardVerticalOffset={Platform.OS==='ios'?64:0}>
        <FlatList
          ...
          renderItem={({ item:m }) => {
            ...
            return (
              <View style={{ alignSelf:isOperator?'flex-end':'flex-start', maxWidth:'82%', marginBottom:8 }}>
                {!isOperator && (
                  <Text style={{ color:C.text2, fontSize:11, marginBottom:3, fontWeight:'600' }}>👤 {userName || 'მომხმარებელი'}</Text>
                )}
                <View style={{
          ...
          ListEmptyComponent={
            <View style={{ padding:30, alignItems:'center' }}>
              ...
              <Text style={{ color:C.text, fontWeight:'700' }}>შეტყობინებები არ არის</Text>
            </View>
          }
        />
        ...
        <View style={{ flexDirection:'row', padding:10, paddingHorizontal:14, borderTopWidth:1, borderTopColor:C.border, backgroundColor:C.surface, alignItems:'flex-end', gap:10 }}>
          <TextInput value={text} onChangeText={setText}
            placeholder="პასუხი..." placeholderTextColor={C.text2} multiline
