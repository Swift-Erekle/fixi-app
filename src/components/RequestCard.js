...
export default function RequestCard({ request, onPress }) {
  ...
  // ✅ FIXED: working favorite toggle — only handymen/companies
  async function toggleFav(e) {
    ...
    setFav(next);
    try {
      await api(`/requests/${request.id}/favorite`, { method: 'POST' });
    } catch { setFav(!next); }
      ...
      <View style={{ padding:16 }}>
        <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
          <View style={{ flexDirection:'row', alignItems:'center', gap:8 }}>
            <View style={{ backgroundColor:color+'20', borderRadius:20, borderWidth:1, borderColor:color+'50', paddingHorizontal:10, paddingVertical:5, flexDirection:'row', alignItems:'center', gap:4 }}>
              <Text style={{ fontSize:12 }}>{catIcon}</Text>
              <Text style={{ color, fontSize:12, fontWeight:'700' }}>{request.category||''}</Text>
            </View>
            {isNew && (
          ...
          {/* ✅ Only shown and functional for handymen/companies */}
          {isWorker ? (
        ...
        <View style={{ flexDirection:'row', alignItems:'center', gap:14, flexWrap:'wrap' }}>
          ...
          {request.budget && <Text style={{ color:'#10b981', fontSize:13, fontWeight:'700' }}>💰 {request.budget}₾</Text>}
